...
This guide is intended to teach you some basics to help you to [troubleshoot performance issues](docs/performance.md#profiling), as well as discuss [common sources of problems and their suggested solutions](docs/performance.md#common-sources-of-performance-problems).
...
![](/website/docsite/docs/assets/PerfUtil.png)
...
Use the new [`FlatList`](docs/flatlist.md) or [`SectionList`](docs/sectionlist.md) component instead. Besides simplifying the API, the new list components also have significant performance enhancements, the main one being nearly constant memory usage for any number of rows.

If your [`FlatList`](docs/flatlist.md) is rendering slow, be sure that you've implemented [`getItemLayout`](docs/flatlist.md#getitemlayout) to optimize rendering speed by skipping measurement of the rendered items.
...
Solving this is one of the main goals behind the new [React Navigation](docs/navigation.md) library. The views in React Navigation use native components and the [`Animated`](docs/animated.md) library to deliver 60 FPS animations that are run on the native thread.
